...
class TransacaoService {
  criar(transacao) {
    ...
    return new Promise((resolve, reject) => {
      ...
      stmt.run(descricao, valor, tipo, categoria, data, function(err) {
  ...
  atualizar(id, transacao) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        stmt.run(descricao, valor, tipo, categoria, data, id, function(err) {
          if (err) return reject(err);
          db.get('SELECT * FROM transacoes WHERE id = ?', [id], (err, row) => {
            if (err) return reject(err);
            resolve(row);
    ...
    return new Promise((resolve, reject) => {
      ...
      `, [], (err, resultados) => {
        ...
        resultados.forEach(row => {
          ...
          if (row.tipo === 'receita') meses[row.mes].receitas = row.total;
          if (row.tipo === 'despesa') meses[row.mes].despesas = row.total;
